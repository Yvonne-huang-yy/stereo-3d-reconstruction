...
def data_normalization(data, omin, omax, imin=None, imax=None):
    'Data normalization.'
    imax = imax if imax is not None else np.max(data)  
    imin = imin if imin is not None else np.min(data)  
...
def image_boxes_resize_padding_inverse(image_size, input_size, boxes=None):
    'Image boxes resize padding inverse.'
...
def draw_image_points_arrowed_lines(image, points,
                                    ...
                                    drawType="simple",
                                    reverse=False):
    'Draw image points arrowed lines.\n\n:param image:\n:param pointline: `auto`->pointline = circle_line(len(points), iscircle=True)\n:param color:\n:param texts:\n:param drawType: simple or custom\n:param check:\n:return:'
    points = np.asarray(points, dtype=np.int32)
    thickness = 2
    if texts is None:
        texts = list(range(len(points)))
...
def read_image_base64(image_path, resize_height=None, resize_width=None):
    if resize_height is None and resize_width is None:
        ...
        bgr_image = read_image(image_path, resize_height, resize_width, colorSpace="bgr")
...
def draw_yaws_pitchs_rolls_axis_in_image(image,
                                         ...
                                         centers=None,
                                         size=75):
    'Draw yaws pitchs rolls axis in image.\n\n:param image:\n:param yaws:\n:param pitchs:\n:param rolls:\n:param centers:\n:param size:\n:return:'
...
def draw_yaw_pitch_roll_in_right_axis(image,
                                      ...
                                      vis=True,
                                      size=75):
    'Draw yaw pitch roll in right axis.\n\n:param image: BGR Image\n:param center:\n:param vis:\n:param size:\n:return:\nReference: https://blog.csdn.net/a812073479/article/details/100103442'
...
def draw_yaw_pitch_roll_in_left_axis(image,
                                     ...
                                     vis=True,
                                     size=75):
    'Draw yaw pitch roll in left axis.'
    text = "(pitch,yaw,roll)=({:3.3f},{:3.3f},{:3.3f})".format(pitch, yaw, roll)
    if center is None:
        h, w, c = image.shape
        center = (w / 2, h / 2)
